...
export async function submitApplicationAction(
  ...
  try {
    ...
    const hasCoFounderRaw = payload?.founder?.hasCoFounder as
      | "yes"
      | "no"
      | undefined;
    const hasCoFounder = hasCoFounderRaw === "yes";

    // fundSpecific (may be missing for older clients)
    ...
    const { error: insErr } = await supabase
      ...
      .insert({
        ...
        cofounder_first_name: hasCoFounder
          ? payload?.founder?.cofounderFirstName ?? null
          : null,
        ...
        outlander_has_technical_10pct:
          typeof outlander?.hasTechnicalLead10pct === "boolean"
            ? outlander.hasTechnicalLead10pct
            : null,
      })
      .single();
